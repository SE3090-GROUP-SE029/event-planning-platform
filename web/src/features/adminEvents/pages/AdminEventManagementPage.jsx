...
const columns = [
  ['id', 'Event Id'], ['owner', 'Owner'], ['eventType', 'Event Type'],
  ['guestCount', 'Guest Count'], ['budget', 'Budget'], ['status', 'Status'],
  ['preferredDate', 'Preferred Date'], ['createdAt', 'Created Date'],
];
...
export default function AdminEventManagementPage() {
  ...
  const [filters, setFilters] = useState({
    search: '', status: '', eventType: '', ownerId: '', dateFrom: '', dateTo: '',
  });
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} flexWrap="wrap" gap={2}>
              <FormControl size="small" sx={{ minWidth: 150 }}>
                ...
                <Select value={filters.status} label="Status" onChange={setFilter('status')}>
                  ...
                  {EVENT_STATUSES.map((value) => <MenuItem key={value} value={value}>{enumLabel(value)}</MenuItem>)}
                </Select>
              </FormControl>
              <FormControl size="small" sx={{ minWidth: 150 }}>
                ...
                <Select value={filters.eventType} label="Event type" onChange={setFilter('eventType')}>
                  ...
                  {EVENT_TYPES.map((value) => <MenuItem key={value} value={value}>{enumLabel(value, EVENT_TYPES.map((item) => item.replaceAll('_', ' ')))}</MenuItem>)}
                </Select>
              </FormControl>
              <TextField size="small" label="Owner ID" value={filters.ownerId} onChange={setFilter('ownerId')} />
              ...
              <FormControl size="small" sx={{ minWidth: 170 }}>
                ...
                <Select value={sort.sortBy} label="Sort by" onChange={(event) => { setSort((current) => ({ ...current, sortBy: event.target.value })); setPage(0); }}>
                  <MenuItem value="createdAt">Created date</MenuItem>
                  <MenuItem value="preferredDate">Preferred date</MenuItem>
                  <MenuItem value="budget">Budget</MenuItem>
                </Select>
              </FormControl>
              <Button variant="outlined" onClick={() => setSort((current) => ({ ...current, sortOrder: current.sortOrder === 'asc' ? 'desc' : 'asc' }))}>
                {sort.sortOrder === 'asc' ? 'Ascending' : 'Descending'}
              </Button>
            </Stack>
          </CardContent>
        </Card>
        {query.isError && <Alert severity="error" sx={{ mb: 2 }}>{query.error.message}</Alert>}
        <Card sx={{ borderRadius: 3 }}>
          <TableContainer>
            <Table size="small">
              <TableHead><TableRow>{columns.map(([field, label]) => <TableCell key={field} onClick={() => ['createdAt', 'preferredDate', 'budget'].includes(field) && handleSort(field)} sx={{ fontWeight: 700, cursor: ['createdAt', 'preferredDate', 'budget'].includes(field) ? 'pointer' : 'default' }}>{label}</TableCell>)}<TableCell /></TableRow></TableHead>
              <TableBody>
                {query.isLoading ? <TableRow><TableCell colSpan={9} align="center"><CircularProgress sx={{ my: 4 }} /></TableCell></TableRow> : items.map((item) => (
                  <TableRow hover key={item.id}>
                    ...
                    <TableCell>{enumLabel(item.status)}</TableCell>
                    <TableCell>{formatDate(item.preferredDate)}</TableCell>
                    <TableCell>{formatDate(item.createdAt)}</TableCell>
                    <TableCell><Button size="small" onClick={() => navigate(`/admin/events/${item.id}`)}>View</Button></TableCell>
                  </TableRow>
                ))}
                {!query.isLoading && items.length === 0 && <TableRow><TableCell colSpan={9} align="center"><Typography sx={{ py: 4 }} color="text.secondary">No events found.</Typography></TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
          <TablePagination component="div" count={query.data?.totalCount || 0} page={page} onPageChange={(_, next) => setPage(next)} rowsPerPage={pageSize} onRowsPerPageChange={(event) => { setPageSize(Number(event.target.value)); setPage(0); }} rowsPerPageOptions={[10, 25, 50]} />
        </Card>
      </Box>
    </Box>
  );
